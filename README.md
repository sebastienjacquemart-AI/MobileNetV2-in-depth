...
Global Average pooling averages across each channel's entire spatial map. For each channel, take the mean of its 5×5 grid. Result: one number per channel. After GlobalAveragePooling2D, you  have a 1280-element vector per image. Now you want to predict 6 classes instead of just 1 output. Thus: Your Dense layer (6 neurons) should output 6 values, one per class. Apply softmax (either manually or via loss function settings) to turn the 6 logits into probabilities — one per class. Pick the class with the highest probability as your prediction. Each neuron in the Dense layer: Receives the full 1280-element vector (after GlobalAveragePooling). Applies its own set of weights and bias. Outputs one number (logit).
