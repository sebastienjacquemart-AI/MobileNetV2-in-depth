...
Context: In standard 2D convolution, the input volume has dimensions Du×Du×M (width, height, channels), and each filter has dimensions Dr×Dr×M. The depth M of the filter must match the number of input channels, so that the filter can capture information from all channels (for example, all three color channels in an RGB image) at once. Each filter produces one 2D feature map. By using N different filters, the output volume has N channels — one for each filter. The number of filters (like 64) is chosen based on the complexity of the input, the depth of the network, available hardware, and experimentation. Early layers usually have fewer filters to capture simple features, while deeper layers have more filters to capture complex patterns. A common practice is to start with fewer filters (e.g., 32) and double them in deeper layers (e.g., 64, 128, 256). Early layers look at small, local patterns — like edges, corners, textures. These features are simple, so you don’t need many different kinds of filters to capture them. Deeper layers combine many simple patterns to recognize complex, high-level structures (e.g., faces, objects). Complex structures are varied and diverse, so you need more filters to capture all the different possibilities.

The two interpretations of convolution operations: 1. The spatial filtering interpretation: each input channel is convolved separately with the corresponding channel of the filter. For each position, the results from all channels are then summed together to produce a single output value. This process is repeated spatially across the whole input to build the output feature map. For N filters, this operation is performed N times, once for each filter, meaning that every input channel is filtered N times, which can become quite computationally expensive; 2. The linear combinations interpretation: The filter is stepped spatially across the input, and at each location, an inner product (dot product) is taken between the overlapping regions of the input and the filter. Practically, both regions are flattened into vectors, and the dot product computes a single output value for each step.
...
The residual connections: 1. Prevent important features from being destroyed by small layers; 2. make it easier for gradients to flow backward through many layers (solve vanishing gradient (Gradients have a shortcut path: they can flow directly through the addition. Even if the transformation F(x) is poorly conditioned (small gradient), the gradient can still flow through the "identity" part (the skip connection). problem).)
...
Global Average pooling averages across each channel's entire spatial map. For each channel, take the mean of its 5×5 grid. Result: one number per channel. After GlobalAveragePooling2D, you  have a 1280-element vector per image. Now you want to predict 6 classes instead of just 1 output. Thus: Your Dense layer (6 neurons) should output 6 values, one per class. Apply softmax (either manually or via loss function settings) to turn the 6 logits into probabilities — one per class. Pick the class with the highest probability as your prediction. Each neuron in the Dense layer: Receives the full 1280-element vector (after GlobalAveragePooling). Applies its own set of weights and bias. Outputs one number (logit). softmax normalizes logits into probabilities. For example, for a logit: e^logit/sum(logits)
